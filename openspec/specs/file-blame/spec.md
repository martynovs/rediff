# file-blame Specification

## Purpose
Per-line commit attribution for a file at a committed revision: the blame peek's gutter, its
off-thread computation, its compact relative ages, and the path from a blamed line to that commit's
message — so "who changed this line, and why" is answerable without leaving the diff.
