...
## Purpose
The non-interactive commands over the review store — requesting a human review, reporting what state one is in, and draining its feedback as JSON — together with the canonical target encoding that lets a drained anchor be resolved against the changeset it was written against, so an agent can ask to be reviewed and read the answer back without a terminal.
